...
/*this recursive functions identify the two nodes that are out of order and swaps them*/
    Queue *q;
    q = malloc(sizeof(BTNode));
    q->size = 0;
    q->head = NULL;
    q->tail = NULL;
    Stack* s;
    ...
    s->top = NULL;
    BTNode *curr = root;
    while (curr != NULL || isEmptyStack(s) == 0)
    {
        while (curr !=  NULL)
        {
            push(s,curr);
    ...
    {
        BTNode *node_1 = getFront(*q);
        dequeue(q);
        int size = q->size;
        int i = 0;
        for (i = 0; i < size; i++)
        {
            BTNode *node_2 = getFront(*q);
            dequeue(q);
            if (node_1->item > node_2->item)
            {
                int temp = node_1->item;
